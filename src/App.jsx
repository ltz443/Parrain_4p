...
<div style={{ fontSize: 12, fontWeight: 700, color: ‘#4FFFA0’, marginBottom: 10, textTransform: ‘uppercase’, letterSpacing: ‘0.06em’ }}>Conditions</div>
{o.conditions.map((c, i) => (
<div key={i} style={{ display: ‘flex’, gap: 8, alignItems: ‘flex-start’, marginBottom: 8 }}>
<span style={{ color: ‘#4FFFA0’, fontSize: 12, marginTop: 2 }}>✓</span>
<span style={{ fontSize: 13, color: ‘#8A95AA’, lineHeight: 1.5 }}>{c}</span>
</div>
))}
</div>
{o.type === ‘code’ && (
...
```
  <div style={{ background: '#111318', border: '1px solid #1A1E2A', borderRadius: 16, padding: '20px', marginTop: 24, textAlign: 'center' }}>
    <h3 style={{ fontSize: 16, fontWeight: 900, color: '#4FFFA0', marginBottom: 8 }}>Newsletter Exclusive 📥</h3>
    <p style={{ fontSize: 12, color: '#8A95AA', marginBottom: 16 }}>Recois les nouveaux bons plans avant tout le monde.</p>
    {newsletterOk ? (
      <div style={{ color: '#4FFFA0', fontWeight: 800, fontSize: 14 }}>✅ Merci, tu es inscrit !</div>
    ) : (
      <div style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          style={{ flex: 1, background: '#0A0B0F', border: '1.5px solid #1E2230', borderRadius: 8, color: '#FFF', padding: '10px 12px', outline: 'none', fontSize: 14 }}
        />
        <button
          onClick={() => { if (email) setNewsletterOk(true); }}
          style={{ background: '#4FFFA0', border: 'none', borderRadius: 8, color: '#0A0B0F', fontWeight: 800, padding: '0 16px', cursor: 'pointer', fontSize: 14 }}>
          OK
        </button>
      </div>
    )}
...
function PageAvis() {
const avis = [
{ nom: “Lucas”, date: “Il y a 2 jours”, texte: “Super rapide pour Hello Bank, j’ai recu mes 80€ comme prevu ! 🔥”, note: “⭐⭐⭐⭐⭐” },
{ nom: “Thomas”, date: “Il y a 1 semaine”, texte: “Je ne connaissais pas du tout les offres de parrainage avant. J’ai teste avec une seule offre pour voir, je valide.”, note: “⭐⭐⭐⭐⭐” },
];

return (
<div style={{ maxWidth: 480, margin: ‘0 auto’, padding: ‘16px’ }}>
<h2 style={{ fontSize: 20, fontWeight: 900, color: ‘#4FFFA0’, marginBottom: 20, textAlign: ‘center’ }}>Avis de la Communaute</h2>
{avis.map((a, i) => (
